	...
	UIronboundWeaponDefinition* NewDefinition)
{
	if (!FighterMesh ||
		...
		!NewDefinition ||
		!NewDefinition->Mesh ||
		!FighterMesh->DoesSocketExist(
			NewDefinition->HandBone))
	{
		return false;
	}
	...
	Weapon->SetPhysicsLinearVelocity(
		FVector::ZeroVector);

	Weapon->SetPhysicsAngularVelocityInDegrees(
		FVector::ZeroVector);
	...
	UE_LOG(
		...
		Log,
		TEXT(
			"Equipment: %s equipped %s revision %d"),
	...
	FBladeTrajectory& OutTrajectory)
{
	...
	/*
	 * Sampling parameters no longer belong in the cache key because the
	 * trajectory analyzer owns them.
	 */
	const FString Key =
		FString::Printf(
			TEXT("%s|%s|%s"),
	...
	if (const FBladeTrajectory* Cached =
		Trajectories.Find(Key))
	{
		OutTrajectory =
			*Cached;
	...
	if (!UIronboundTrajectoryLibrary::
